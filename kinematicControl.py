...
def computeError(customDynamicsAPI, robotName, efLink, efPositionInLink, efOrientationInLink, targetPosition, targetOrientation, efBaseLinearVelocity, efBaseAngularVelocity, linearD=0.1, angularD=0.1):
    ...
    e = numpy.concatenate((positionE - efBaseLinearVelocity, orientationE - efBaseAngularVelocity))
    return e
...
def jacobianTranspose(customDynamicsAPI, robotName, efLink, efPositionInLink, efOrientationInLink, targetPosition, targetOrientation, usableDoFs, efBaseLinearVelocity, efBaseAngularVelocity, dampingSq=1.0, linearD=0.1, angularD=0.1, mobileBase=False):
    e = computeError(customDynamicsAPI, robotName, efLink, efPositionInLink, efOrientationInLink, targetPosition, targetOrientation, efBaseLinearVelocity, efBaseAngularVelocity, linearD, angularD)
    J, Jt = computeAvailableJacobian(customDynamicsAPI, robotName, efLink, efPositionInLink, usableDoFs, mobileBase=mobileBase)
...
def jacobianPseudoinverseDLS(customDynamicsAPI, robotName, efLink, efPositionInLink, efOrientationInLink, targetPosition, targetOrientation, usableDoFs, efBaseLinearVelocity, efBaseAngularVelocity, dampingSq=1.0, linearD=0.1, angularD=0.1, mobileBase=False):
    e = computeError(customDynamicsAPI, robotName, efLink, efPositionInLink, efOrientationInLink, targetPosition, targetOrientation, efBaseLinearVelocity, efBaseAngularVelocity, linearD, angularD)
...
def isAKinematicControllableThing(getFn):
    return getFn(('fn', 'kinematicControl'), None) is True
...
def updateKinematicControl(name, customDynamicsAPI):
    ...
    dofSet = set(dofs)
    mobileBase = customDynamicsAPI['getObjectProperty']((name,) 'immobile', False)
    for ef in fnKinematicControl.get('endEffectors', []):
        ...
        if efTarget is not None:
            ...
            for d in dofs:
                if d in dofSet:
                    force = getDictionaryEntry(fnKinematicControl, ('maxForce', d), 200)
