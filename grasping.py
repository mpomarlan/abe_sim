...
def isAGraspingConstraint(getFn):
    return getFn(('fn', 'graspingConstraint'), None) is True
...
def isAGraspingThing(getFn):
    return getFn(('fn', 'grasping'), None) is True
...
def updateGrasping(name, customDynamicsAPI):
    ...
    efs = getDictionaryEntry(fnGrasping, ('graspingEffectors',), [])
    newActuallyGrasped = {}
    for ef in efs:
        graspingActivationRadius = getDictionaryEntry(fnGrasping, ('graspingActivationRadius', ef), 0.1)
        graspingDeactivationRadius = getDictionaryEntry(fnGrasping, ('graspingDeactivationRadius', ef), 0.1)
        ...
        # check whether intended grasped items are not actually grasped, but close enough; if so, add a constraint to make them grasped
            if (e not in actuallyGraspedEF) and (graspingActivationRadius > customDynamicsAPI['getDistance']((name, ef), e, graspingDeactivationRadius)):
                child = e[0]
                if 1 == len(e):
                ...
                customDynamicsAPI['addObject']({'name': constraintName, 'simtype': 'kcon', 'parent': name, 'child': child, 'parentLink': ef, 'childLink': childLink, 'jointType': 'fixed', 'maxForce': maxForce})
