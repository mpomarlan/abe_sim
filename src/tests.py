...
c = w.addPObjectOfType("knife", CookingKnife, [0, 0.4, 0.5], [0, 0, 0, 1])
b = w.addPObjectOfType("bread", Bread, [0.6, 0.23, 0.45], [0,0,0.707,0.707])
#s = w.addPObjectOfType("slice", BreadSlice, [0.8, 0.4, 0.5], [0,0,0.707,0.707])
...
a.applyRigidBodyControls([{"jointTargets": {"hand_left_y_to_hand_left_z": (-0.0,0,1), "hand_left_x_to_hand_left_y": (0,0,1)}, "+constraints": [], "-constraints": []}])
for k in range(100):
    w.update()

a.applyRigidBodyControls([{"jointTargets": {"world_to_base_x": (0,0,1), "hand_left_base_to_hand_left_x": (0.5,0,1)}, "+constraints": [], "-constraints": []}])
for k in range(100):
    w.update()
    time.sleep(0.1)
...
a.applyRigidBodyControls([{"jointTargets": {"base_x_to_base_y": (0,0,1), "hand_left_x_to_hand_left_y": (-0.05,0,1), "hand_left_y_to_hand_left_z": (0.05,0,1)}, "+constraints": [], "-constraints": []}])
for k in range(100):
    w.update()
    time.sleep(0.1)

a.applyRigidBodyControls([{"jointTargets": {"world_to_base_x": (0,0,1), "hand_left_base_to_hand_left_x": (0.7,0,1)}, "+constraints": [], "-constraints": []}])
for k in range(150):
    w.update()
    time.sleep(0.1)
